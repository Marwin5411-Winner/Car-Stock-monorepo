...
BACKUP_COUNT=$(ls -1 "$BACKUP_DIR"/car-stock_*.dump 2>/dev/null | wc -l)
if [ "$BACKUP_COUNT" -gt "$MAX_BACKUPS" ]; then
  REMOVE_COUNT=$((BACKUP_COUNT - MAX_BACKUPS))
  echo "[$(date)] Rotating backups: removing $REMOVE_COUNT oldest..."
  ls -1t "$BACKUP_DIR"/car-stock_*.dump | tail -n "$REMOVE_COUNT" | xargs rm -f
fi

echo "[$(date)] Backup rotation complete. Current backups:"
ls -lh "$BACKUP_DIR"/car-stock_*.dump 2>/dev/null || echo "  (none)"
